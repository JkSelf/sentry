    ...
    while (true) {
      try {
        ...
        if (!(e.getTargetException() instanceof SentryHdfsServiceException)) {
          throw new SentryHdfsServiceException("Error in Sentry HDFS client", e.getTargetException());
        } else {
        ...
        throw new SentryHdfsServiceException("Error connecting to sentry service " + e1.getMessage(), e1);
      }
    }
    return result;
  }
